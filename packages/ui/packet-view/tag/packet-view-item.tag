...
<li>
  <p class="label list-item" onclick={ toggle } range={ field.range } oncontextmenu={ context } onmouseover={ fieldRange } onmouseout={ rangeOut }>
    <i class="fa fa-circle-o" show={ !field.items.length  }></i>
    <i class="fa fa-arrow-circle-right" show={ field.items.length && !show }></i>
    <i class="fa fa-arrow-circle-down" show={ field.items.length && show }></i>
  ...
  <ul show={ field.items.length && show }>
    <packet-view-item each={ field in field.items } layer={ parent.layer } parentVal={ parent.val } parent={ parent.field } path={ parent.path }></packet-view-item>
  </ul>
</li>
